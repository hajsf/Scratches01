...
const boundaryWord = "IMGBOUNDARY" // "MJPEGBOUNDARY"
const headerf = "\r\n" +
	"--" + boundaryWord + "\r\n" +
...
// UpdateIMG pushes a new IMG frame onto the clients.
func (s *Stream) UpdateIMG(img []byte) {
	header := fmt.Sprintf(headerf, len(img))
	if len(s.frame) < len(img)+len(header) {
		s.frame = make([]byte, (len(img)+len(header))*2)
	}

	copy(s.frame, header)
	copy(s.frame[len(header):], img)
